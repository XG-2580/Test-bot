      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            cmdroles = client.settings.get(message.guild.id, "cmdadminroles.copymessage");
            cmdrole = [];
          ...
          case 33:
            ...
            channel.messages.fetch(id).then(function (msg) {
              if (msg.content) {
                message.author.send("```".concat(prefix, "say ").concat(msg.content, "```"));
              }

              if (msg.embeds[0]) {
                var embed = msg.embeds[0];
                message.author.send("```".concat(prefix).concat(embed.image ? embed.image.url ? "img" : "" : "", "embed ").concat(embed.title ? embed.title : "").concat(embed.image ? embed.image.url ? "++" + embed.image.url : "" : "", "++").concat(embed.description ? embed.description : "", "```"));
              }

              return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle("Check your DMS!"));
            })["catch"](function (e) {
