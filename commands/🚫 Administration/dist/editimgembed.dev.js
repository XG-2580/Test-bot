      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            cmdroles = client.settings.get(message.guild.id, "cmdadminroles.editimgembed");
            cmdrole = [];
          ...
          case 27:
            ...
            return _context.abrupt("return", message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle("You didn't provide an Old Message Id, nor a Title, nor a Description, nor Image").setDescription("Usage: `".concat(prefix, "editimgembed <OLDMESSAGE_ID> ++ <TITLE> ++ <IMAGEURL> ++ <DESCRIPTION>`"))));
          ...
          case 29:
            ...
            message.channel.messages.fetch(oldembedid).then(function (msg) {
              msg.edit({
                embed: new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setFooter(es.footertext, es.footericon).setImage(image ? image.includes("http") ? image : message.author.displayAvatarURL : message.author.displayAvatarURL).setTitle(title ? title.substr(0, 256) : "").setDescription(desc ? desc.substr(0, 2048) : "")
              });
            });
