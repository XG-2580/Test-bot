...
module.exports = {
  ...
  run: function run(client, message, args, cmduser, text, prefix) {
    var es, adminroles, cmdroles, cmdrole, _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, r, warnmember, memberPosition, moderationPosition, warnIDs, toaddids, channel;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            cmdroles = client.settings.get(message.guild.id, "cmdadminroles.removeallwarns");
            cmdrole = [];
          ...
          case 39:
            console.log(warnIDs);
            toaddids = warnIDs.filter(function (id) {
              return client.modActions.get(id).guild != message.guild.id;
            });
            console.log(toaddids);
            client.userProfiles.set(warnmember.user.id, toaddids, 'warnings');
            warnmember.send(new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setFooter(es.footertext, es.footericon).setAuthor("".concat(message.author.tag, " removed all ").concat(warnIDs.length - toaddids.length, " Warnings of **`").concat(message.guild.name, "`** from you!"), message.author.displayAvatarURL({
              dynamic: true
            })))["catch"](function (e) {
              return console.log(e.message);
            });
            message.channel.send(new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setFooter(es.footertext, es.footericon).setTitle("Removed all ".concat(warnIDs.length - toaddids.length, " Warnings of **`").concat(message.guild.name, "`** from ").concat(warnmember.user.tag)));
    ...
    }, null, null, [[1, 64], [9, 13, 17, 25], [18,, 20, 24], [34, 58], [46, 53]]);
  }
};