      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 24:
            ...
            return _context.abrupt("return", message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter("He/She has: ".concat(client.userProfiles.get(warnmember.id, 'warnings') ? client.userProfiles.get(warnmember.id, 'warnings').length : 0, " Global Warns"), "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/joypixels/275/globe-with-meridians_1f310.png").setTitle("**`".concat(warnmember.username, "`** has no Warnings\nin **`").concat(message.guild.name, "`**"))));

          case 31:
            warnembed = new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setFooter("He/She has: ".concat(client.userProfiles.get(warnmember.id, 'warnings') ? client.userProfiles.get(warnmember.id, 'warnings').length : 0, " Global Warns"), "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/joypixels/275/globe-with-meridians_1f310.png").setTitle("[".concat(warnings.length, "] Warnings of: **`").concat(warnmember.tag, "`**\nin **`").concat(message.guild.name, "`**"));
            string = "";

            for (i = 0; i < warnings.length; i++) {
              string += "================================\n**Warn Id:** `".concat(i, "`\n**Warned at:** `").concat(warnings[i].when, "`\n**Warned by:** `").concat(message.guild.members.cache.get(warnings[i].moderator) ? message.guild.members.cache.get(warnings[i].moderator).user.tag : warnings[i].moderator, "`\n**Reason:** `").concat(warnings[i].reason.length > 50 ? warnings[i].reason.substr(0, 50) + " ..." : warnings[i].reason, "`\n");
            }
