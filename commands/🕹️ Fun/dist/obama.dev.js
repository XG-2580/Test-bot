      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 5:
            ...
            return _context.abrupt("return", tempmsg.edit(tempmsg.embeds[0].setTitle("You did not enter a Valid Text!").setColor("RED").setDescription("Useage: `".concat(prefix, "obama @User <TEXT>`")))["catch"](function (e) {
              return console.log("Couldn't delete msg, this is for preventing a bug".gray);
            }));

          case 12:
            //get the memer image
            client.memer.obama(text, avatar).then(function (image) {
              //make an attachment
              var attachment = new MessageAttachment(image, "obama.png"); //delete old message

              tempmsg["delete"](); //send new Message

              message.channel.send(tempmsg.embeds[0].setAuthor("Meme for: ".concat(user.tag), avatar).setImage("attachment://obama.png").attachFiles(attachment))["catch"](function (e) {
                return console.log("Couldn't delete msg, this is for preventing a bug".gray);
              });
            });
