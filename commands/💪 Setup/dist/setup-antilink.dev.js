      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            temptype = "";
            _context.next = 8;
            return regeneratorRuntime.awrap(message.channel.send(new Discord.MessageEmbed().setTitle("What do you want to do?").setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setDescription("1\uFE0F\u20E3 **== ** ".concat(client.settings.get(message.guild.id, "antilink").enabled ? "**\`❌ Disable\`** AntiLinks" : "**\`✔️ Enable\`** AntiLinks", "\n\n2\uFE0F\u20E3 **== Add** White listed **Channels**\n\n3\uFE0F\u20E3 **== Remove** White listed **Channels**\n\n\uD83D\uDCD1 **== Show Settings**\n\n\n\n*React with the Right Emoji according to the Right action*")).setFooter(es.footertext, es.footericon)));
          ...
          case 23:
            ...
            return _context.abrupt("return", message.reply({
              embed: new Discord.MessageEmbed().setTitle("".concat(client.settings.get(message.guild.id, "antilink").enabled ? "**Enabled** AntiLinks" : "**Disabled** AntiLinks")).setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setDescription("".concat(client.settings.get(message.guild.id, "antilink").enabled ? "**I will now prevent Users to send** Links" : "Everyone can send Links!").substr(0, 2048)).setFooter(es.footertext, es.footericon)
            }));
          ...
          case 37:
            ...
            return regeneratorRuntime.awrap(tempmsg.channel.awaitMessages(function (m) {
              ...
              if (channel) {
                var antisettings = client.settings.get(message.guild.id, "antilink.whitelistedchannels");
                if (antisettings.includes(channel.id)) return message.reply({
                  embed: new Discord.MessageEmbed().setTitle("ERROR | The Channel: `".concat(channel.name, "` is already added to the Anti Links Whitelist")).setColor(es.wrongcolor).setFooter(es.footertext, es.footericon)
                });

                try {
                  client.settings.push(message.guild.id, channel.id, "antilink.whitelistedchannels");
                  return message.reply({
                    embed: new Discord.MessageEmbed().setTitle("The Channel: `".concat(channel.name, "` is now registered as an Whitelisted Anti Links Channel")).setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setDescription("Every single Channel:\n<#".concat(client.settings.get(message.guild.id, "antilink.whitelistedchannels").join(">\n<#"), ">\nis not a checked by the Anti Links System").substr(0, 2048)).setFooter(es.footertext, es.footericon)
                  });
                } catch (e) {
                  return message.reply({
          ...
          case 47:
            ...
            return regeneratorRuntime.awrap(tempmsg.channel.awaitMessages(function (m) {
              ...
              if (channel) {
                var antisettings = client.settings.get(message.guild.id, "antilink.whitelistedchannels");
                if (!antisettings.includes(channel.id)) return message.reply({
                  embed: new Discord.MessageEmbed().setTitle("ERROR | The Channel: `".concat(channel.name, "` is not added to the Anti Links Whitelist yet")).setColor(es.wrongcolor).setFooter(es.footertext, es.footericon)
                });

                try {
                  client.settings.remove(message.guild.id, channel.id, "antilink.whitelistedchannels");
                  return message.reply(new Discord.MessageEmbed().setTitle("The Channel: `".concat(channel.name, "` is now **NOT** registered as an Whitelisted Anti Links Channel anymore")).setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setDescription("Every single Channel:\n<#".concat(client.settings.get(message.guild.id, "antilink.whitelistedchannels").join(">\n<#"), ">\nis not a checked by the Anti Links System").substr(0, 2048)).setFooter(es.footertext, es.footericon));
                } catch (e) {
          ...
          case 54:
            ...
            thesettings = client.settings.get(message.guild.id, "antilink");
            return _context.abrupt("return", message.reply(new Discord.MessageEmbed().setTitle("\uD83D\uDCD1 Settings of the Anti Ad-Link System").setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setDescription("**Enabled:** ".concat(thesettings.enabled ? "<a:yes:833101995723194437>" : "<:no:833101993668771842>", "\n          \n**Witelisted Channels:** ").concat(thesettings.whitelistedchannels.length > 0 ? "<#".concat(thesettings.whitelistedchannels.join("> | <#"), ">") : "No Channels Whitelisted!", "\n\n**Information:** *Anti Links are not enabled in Tickets from THIS BOT*").substr(0, 2048)).setFooter(es.footertext, es.footericon)));
