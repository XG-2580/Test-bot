...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    return message.channel.send(new MessageEmbed()
        ...
        .setTitle(`<833101993668771842> You didn't pinged to whom you want to transfer`)
    ...
    if(!transferamount)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<833101993668771842> You didn't add the transferamount`)
        .setDescription(`Usage: \`${prefix}transfer <@USER> <Amount>\`\n\n\Example: \`${prefix}transfer <@442355791412854784> 42069\``)
      );
    //if user or the totransfer user is a bot, return error
    if(user.bot || totransfer.bot) return message.reply("<833101993668771842> **A Discord Bot can not have Economy!**")
    ...
    return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<833101993668771842> You can't transfer a negative Amount of Money or no Money, to ${totransfer}`)
      );
    ...
    if(transferamount > data.balance)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<833101993668771842> You can't transfer more Money than you have in your **👛 Pocket (\`${data.balance} 💸\`)**`)
      );
    ...
    return message.reply(new MessageEmbed()
      ...
      .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
      .setTitle(`<a833101995723194437> **You transfered \`${transferamount} 💸\` to \`${totransfer.tag}\`**`)
