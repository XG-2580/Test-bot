  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if(args[0].toLowerCase() == "all"){
      ...
      return message.reply(new MessageEmbed()
        ...
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<a833101995723194437> You deposited **\`${nFormatter(deposited)}💸\`** to your Bank`)
      ...
      return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setTitle(`<833101993668771842> You can't deposit a negative Amount of Money or no Money, to your Bank`)
        );
      ...
      if(amount > data.balance)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setTitle(`<833101993668771842> You can't deposit more Money than you have in your **👛 Pocket (\`${nFormatter(data.balance)} 💸\`)**`)
        );
      ...
      return message.reply(new MessageEmbed()
        .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<a833101995723194437> You deposited **\`${amount}💸\`** to your Bank`)
        .setDescription(`**🏦 You now have \`${nFormatter(Math.floor(data.bank))} 💸\` in your Bank**\n\n👛 You now have \`${nFormatter(Math.floor(data.balance))} 💸\` in your Pocket`)
      );
    }
  } catch (e) {
