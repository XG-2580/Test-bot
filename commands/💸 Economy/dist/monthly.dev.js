      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
            ...
            time = duration(timeout - (Date.now() - data.monthly));
            return _context.abrupt("return", message.reply({
              embed: new MessageEmbed().setColor(es.wrongcolor).setFooter(message.author.tag, message.author.displayAvatarURL({
                dynamic: true
              })).setTitle("You've already collected your monthly reward!").setDescription("\uD83D\uDD50 **Try again in ".concat(time.map(function (i) {
                return "`".concat(i, "`");
              }).join(", "), "**\n\n\uD83D\uDC5B You still have `").concat(Math.floor(data.balance), " \uD83D\uDCB8` in your Pocket"))
            }));

          case 15:
            amountarray = [300 * 12.5, 350 * 12.5, 400 * 12.5, 340 * 12.5, 360 * 12.5, 350 * 12.5, 355 * 12.5, 345 * 12.5, 365 * 12.5, 350 * 12.5, 340 * 12.5, 360 * 12.5, 325 * 12.5, 375 * 12.5, 312.5 * 12.5, 387.5 * 12.5];
            ...
            client.economy.set("".concat(message.guild.id, "-").concat(message.author.id), Date.now(), "monthly"); //get the new data
            ...
            return _context.abrupt("return", message.reply(new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setFooter(message.author.tag, message.author.displayAvatarURL({
              dynamic: true
            })).setTitle("You've collected your monthly reward of `".concat(amount, " \uD83D\uDCB8`")).setDescription("\uD83D\uDC5B You now have `".concat(Math.floor(data.balance), " \uD83D\uDCB8` in your Pocket"))));
