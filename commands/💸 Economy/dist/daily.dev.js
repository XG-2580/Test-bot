      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
            ...
            timeout = 86400000; //if the user is on delay return some error
            ...
            time = duration(timeout - (Date.now() - data.daily));
            return _context.abrupt("return", message.reply({
              embed: new MessageEmbed().setColor(es.wrongcolor).setFooter(message.author.tag, message.author.displayAvatarURL({
                dynamic: true
              })).setTitle("You've already collected your daily reward!").setDescription("\uD83D\uDD50 **Try again in ".concat(time.map(function (i) {
                return "`".concat(i, "`");
          ...
          case 15:
            ...
            client.economy.set("".concat(message.guild.id, "-").concat(message.author.id), Date.now(), "daily"); //get the new data
            ...
            return _context.abrupt("return", message.reply(new MessageEmbed().setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setFooter(message.author.tag, message.author.displayAvatarURL({
              dynamic: true
            })).setTitle("You've collected your daily reward of `".concat(amount, " \uD83D\uDCB8`")).setDescription("\uD83D\uDC5B You now have `".concat(nFormatter(Math.floor(data.balance)), " \uD83D\uDCB8` in your Pocket"))));
