    ...
    try {
      ...
      if (!args[0])
        return message.channel.send(new MessageEmbed()
          .setFooter(es.footertext, es.footericon)
          .setColor(es.wrongcolor)
          .setTitle(`<833101993668771842> Please include to which track you want to jump`)
      ...
      if (isNaN(args[0]))
        return message.channel.send(new MessageEmbed()
          .setFooter(es.footertext, es.footericon)
          .setColor(es.wrongcolor)
          .setTitle(`<833101993668771842> It has to be a queue **Number**`)
        );
      ...
      if (Number(args[0]) > player.queue.size)
        return message.channel.send(new MessageEmbed()
          .setFooter(es.footertext, es.footericon)
          .setColor(es.wrongcolor)
          .setTitle(`<833101993668771842> That song is not in the queue, sorry!`)
        );
      ...
      return message.channel.send(new MessageEmbed()
        .setTitle(`<a833101995723194437> Jumped to the: \`${args[0]}\` Song`)
